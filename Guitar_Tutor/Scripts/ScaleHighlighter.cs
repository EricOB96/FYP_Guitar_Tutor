...
public partial class ScaleHighlighter : Node
{
    ...
    // Material paths for different types of notes
    [Export]
    ...
    public override void _Ready()
    {
        ...
        {
            GD.PushError("Failed to find ScaleLibrary node. Check the path in ScaleHighlighter.");
        }
        else
        {
            GD.Print("ScaleHighlighter initialized!");
        }
    }

    /// <summary>
    /// Highlights the notes of the specified scale on the fretboard
    /// </summary>
    public void HighlightScale(string rootNote, string scaleType, int? numFrets = null)
    {
        int fretsToUse = numFrets ?? _numFrets;

        GD.Print($"Highlighting scale: {rootNote} {scaleType}");
        ...
        GD.Print($"Description: {scale.Description}");
        ...
        for (int stringNum = 6; stringNum >= 1; stringNum--)  // Starting from String6 (Low E) down to String1 (High E)
        {
            ...
            for (int fret = 0; fret <= fretsToUse; fret++)  // +1 to include the open string 
            {
    ...
    /// </summary>
    private void ApplyMaterial(Node node, string materialType)
    {
        try
        {
    ...
    private void ResetFretboard(int numFrets)
    {
        ...
        {
            Node3D node = GetNodeOrNull<Node3D>($"../FretboardV3/ScaleNodes/OpenString/String{stringNum}");
            if (node != null)
            {
        ...
        for (int fret = 1; fret <= numFrets; fret++)
        {
            for (int stringNum = 6; stringNum >= 1; stringNum--)
            {
                Node3D node = GetNodeOrNull<Node3D>($"../FretboardV3/ScaleNodes/Fret{fret}/String{stringNum}");
                if (node != null)
                {
                    ApplyMaterial(node, "default");
                }
